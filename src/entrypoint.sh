...
if ! tfsec --format json "$PATH_TO_SCAN" > tfsec_output.json 2>tfsec_error.txt; then
    echo "tfsec completed with findings"
fi
if [ ! -s tfsec_output.json ]; then
    echo "Error: No output generated by tfsec"
    if [ -f tfsec_error.txt ]; then
        echo "Error output:"
        cat tfsec_error.txt
    fi
    exit 1
